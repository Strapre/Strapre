...
export default function CategoryPage() {
  ...
  const fetchCategoryProducts = async (categoryId: string, page = 1) => {
    setLoading(true)
    try {
      let url = `${ENDPOINTS.productsByCategory(categoryId)}?page=${page}`
  ...
  const handleStateChangeFromSelect = (stateId: string) => {
    if (stateId === "all") {
  ...
  const handleLGAChangeFromSelect = (lgaId: string) => {
    if (lgaId === "all") {
      setSelectedLGA(null)
    } else {
              ...
              <div className="flex items-center space-x-4 justify-between mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <div className="hidden md:flex items-center space-x-3">
                    <Select onValueChange={handleStateChangeFromSelect} value={selectedState?.id || "all"}>
                    ...
                    <Select onValueChange={handleLGAChangeFromSelect} value={selectedLGA?.id || "all"} disabled={!selectedState}>
                      <SelectTrigger className="w-32">
